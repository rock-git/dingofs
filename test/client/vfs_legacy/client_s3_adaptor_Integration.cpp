...
#include "client/vfs_legacy/mock_kvclient.h"
...
Status S3Upload(const std::string& name, const char* buf, size_t len) {
...
Status S3Download(std::string name, uint64_t offset, uint64_t len, char* buf) {
  S3Data& tmp = gObjectDataMaps[name];
...
TEST_F(ClientS3IntegrationTest, test_truncate_small1) {
  ...
  len = 1 * 1024 * 1024;
  DINGOFS_ERROR ret = s3ClientAdaptor_->Truncate(inode.get(), len);
...
TEST_F(ClientS3IntegrationTest, test_truncate_small2) {
  ...
  DINGOFS_ERROR ret = s3ClientAdaptor_->Truncate(inode.get(), truncateLen);
  ASSERT_EQ(DINGOFS_ERROR::OK, ret);
  ASSERT_EQ(3 * len, s3ClientAdaptor_->GetFsCacheManager()->GetDataCacheSize());
...
TEST_F(ClientS3IntegrationTest, test_truncate_small3) {
  ...
  DINGOFS_ERROR ret = s3ClientAdaptor_->Flush(inode->GetInodeId());
...
TEST_F(ClientS3IntegrationTest, test_fssync_success_and_fail) {
  ...
  DINGOFS_ERROR ret = s3ClientAdaptor_->FsSync();
  ASSERT_EQ(DINGOFS_ERROR::OK, ret);
  ASSERT_EQ(0, fsCacheManager->GetDataCacheNum());
...
TEST_F(ClientS3IntegrationTest, test_write_read_remotekvcache) {
  ...
  // write data and prepare for flush
  {
    ...
    int ret = s3ClientAdaptor_->Write(inodeId, offset_0, len, buf);
    ASSERT_EQ(len, ret);
  ...
  // flush data
  {
    ...
    DINGOFS_ERROR res = s3ClientAdaptor_->Flush(inodeId);
    ASSERT_EQ(DINGOFS_ERROR::OK, res);
  }
